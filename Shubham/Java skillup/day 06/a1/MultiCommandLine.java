...
public class MultiCommandLine {
	...
	void doOperation(String[] args){	//Getting the first argument of args
		arrInt = new Integer[args.length];
		arrStr = new String[args.length];
		String firstArgument = args[0];
		int intLength = args.length,intCnt, intSum = 0;
		boolean flag,isValid = false;
		arrInt = new Integer[args.length];
	...
	void printStringArray(){
		for(int i = 0; i < arrStr.length-1 ; i++ ){
			if(arrStr[i] != null);
				System.out.println("Sum of Integer Numbers = " + arrStr[i].toLowerCase() );
		}
	}
}
